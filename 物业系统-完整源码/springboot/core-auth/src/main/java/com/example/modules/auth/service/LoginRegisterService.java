...
//    private static final long TOKEN_EXPIRE_SECONDS = 60 * 60 * 8; // 8小时
    ...
    public User login(User loginUser) {
        ...
        if (dbUser == null) {
            // 尝试用手机号登录
            dbUser = loginRegisterMapper.selectByPhone(username);
        }
        if (dbUser == null) {
        ...
        try {
            // 统一用数据库中的 username 做认证，兼容手机号登录
            authenticationManager.authenticate(
                    new UsernamePasswordAuthenticationToken(dbUser.getUsername(), password)
            );
    ...
    // 发送短信验证码（演示模式：验证码直接返回，生产环境应接入短信服务商）
    public String sendSmsCode(String phone) {
        User dbUser = loginRegisterMapper.selectByPhone(phone);
        if (dbUser == null) {
            throw new CustomException("500", "该手机号未注册");
        }
        // 生成6位随机验证码
    ...
    public User loginBySms(String phone, String code) {
        String cached = (String) redisTemplate.opsForValue().get("sms:code:" + phone);
        if (cached == null) {
            throw new CustomException("500", "验证码已过期，请重新获取");
        }
