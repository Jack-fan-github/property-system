...
public class UserAdminController {
    ...
    // GET方式登录：微信小程序wx.request的cookie机制不可靠，
    // 云代理302重定向会把POST转GET导致请求体丢失。
    // GET请求参数在URL里，302重定向不丢数据。
    ...
    public Result sendSmsCode(@RequestBody Map<String, String> body) {
        ...
        if (phone == null || phone.isEmpty()) {
            throw new CustomException("400", "手机号不能为空");
        }
        String code = loginRegisterService.sendSmsCode(phone);
        // 演示环境直接返回验证码，生产环境应删除此行并接入短信平台
    ...
    public Result smsLogin(@RequestBody Map<String, String> body) {
        ...
        if (phone == null || code == null) {
            throw new CustomException("400", "手机号和验证码不能为空");
        }
        User db = loginRegisterService.loginBySms(phone, code);
        return Result.success(db);
    }
    ...
    public Result employeeLoginGet(@RequestParam String username, @RequestParam String password) {
        Employee loginEmployee = new Employee();
        loginEmployee.setUsername(username);
        loginEmployee.setPassword(password);
        Employee dbEmployee = loginRegisterService.employeeLogin(loginEmployee);
        return Result.success(dbEmployee);
    }
    // 登出
    @PostMapping("/logout")
